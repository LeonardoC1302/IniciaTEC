...
class GuiasController
{
    ...
    public static function registerasistente(Router $router)
    {
        ...
        $rol = Role::where('nombre', 'Asistente administrativo');
        $estado = UserStatus::where('nombre', 'Activo');
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            //debug($_POST);
            $user->sync($_POST);
            $user->roleId = $rol->id;
            $user->estadoId = $estado->id;
            $user->generatePassword();
            $alerts = $user->validateAsistente();
            //debug($alerts);
            if (empty($alerts)) {
                ...
                $user->hashPassword();
                $userId = $user->save();
                $asistente->usuarioId = $userId['id'];
                $asistente->save();
                header('Location: /guias');
            }
        }

        $router->render('/guias/register-asistente', [
            'alerts' => $alerts
        ]);
    }
}
