...
<div class="add-activity">
    ...
    ?>
    <h1 class="add-activity__title">Modificar Actividad</h1>
    ...
    <form class="add-activity__form" method="POST" enctype="multipart/form-data">
        ...
        <div class="add-activity__form__field" id="estadoDiv">
            <label for="estado" class="add-activity__form__label">Estado</label>
            <select id="estado" name="estadoId" class="add-activity__form__input" required>
                <option value="">-- Seleccione un estado --</option>
                <?php foreach($states as $state): ?>
                    <option <?php echo ($state->id === $activity->estadoId) ? 'selected' : '' ?> value="<?php echo $state->id; ?>"><?php echo $state->nombre; ?></option>
                <?php endforeach; ?>
            </select>
        </div>

        <div class="add-activity__form__field--evidencias <?php echo ($activity->estadoId != 3) ? 'add-activity__form__field--disabled' : ''; ?>" id="divEvidencias">
            <label for="evidencias" class="add-activity__form__label">Evidencias</label>
            <input type="file" id="evidencias" name="evidencias[]" class="add-activity__form__input" accept="image/jpeg, image/png" multiple>
        </div>

        <div class="add-activity__form__field--justificacion <?php echo ($activity->estadoId != 4) ? 'add-activity__form__field--disabled' : ''; ?>" id="divJustificacion">
            <label for="justificacion" class="add-activity__form__label">Justificación</label>
            <textarea id="justificacion" name="justificacion" class="add-activity__form__input--textarea"></textarea>
        </div>
        ...
        <button type="submit" class="add-activity__form__submit">Actualizar</button>
    </form>
</div>